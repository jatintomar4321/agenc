...
const ThemeToggle = () => {
  ...
  return (
    <button
      onClick={toggleTheme}
      className="fixed bottom-12 left-6 z-50 p-2 rounded-full bg-opacity-20 backdrop-blur-sm"
...
const PageContent = () => {
  ...
  return (
    <div
      ref={scrollRef}
      className={`w-full ${bgColor} ${textColor} h-full pt-10 transition-colors duration-300`}
    >
      <Navbar />
      <ThemeToggle />
      ...
      <div
        className={`${bgColor} relative w-full transition-colors duration-300 pb-4`}
      >
        <Slider
          h1="Faster"
          h2="Explore Idea"
